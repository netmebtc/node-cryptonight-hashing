...
inline void cryptonight_single_hash(const uint8_t *__restrict__ input, size_t size, uint8_t *__restrict__ output, cryptonight_ctx **__restrict__ ctx)
{
    ...
    for (size_t i = 0; i < ITERATIONS; i++) {
        ...
        if (IS_MONERO) {
            cryptonight_monero_tweak<VARIANT == xmrig::VARIANT_MOX ? 8 : 3>((uint64_t*)&l0[idx0 & MASK], _mm_xor_si128(bx0, cx));
        } else {
...
inline void cryptonight_double_hash(const uint8_t *__restrict__ input, size_t size, uint8_t *__restrict__ output, cryptonight_ctx **__restrict__ ctx)
{
    ...
    for (size_t i = 0; i < ITERATIONS; i++) {
        ...
        if (IS_MONERO) {
            cryptonight_monero_tweak<VARIANT == xmrig::VARIANT_MOX ? 8 : 3>((uint64_t*)&l0[idx0 & MASK], _mm_xor_si128(bx0, cx0));
            cryptonight_monero_tweak<VARIANT == xmrig::VARIANT_MOX ? 8 : 3>((uint64_t*)&l1[idx1 & MASK], _mm_xor_si128(bx1, cx1));
        } else {
...
#define CN_STEP2(a, b, c, l, ptr, idx)                                 \
    ...
    b = _mm_xor_si128(b, c);                                           \
                                                                       \
    if (IS_MONERO) {                                                 \
        cryptonight_monero_tweak<VARIANT == xmrig::VARIANT_MOX ? 8 : 3>(reinterpret_cast<uint64_t*>(ptr), b); \
    } else {                                                           \
